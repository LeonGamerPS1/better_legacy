...
class PlayState extends MusicBeatState
{
	...
	private function generateSong():Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				swagNote.sustainLength = songNotes[2] is Float ? songNotes[2] : 0;
	...
	public function spawnSustain(parent:Note):Sustain
	{
		var sustain:Sustain = new Sustain(parent);
		parent.sustain = sustain;
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		{
			// FlxG.log.add(i);
			var babyArrow:StrumNote = new StrumNote(0, strumLine.y, i % 4);
			babyArrow.downScroll = PreferencesMenu.getPref('downscroll');
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.setGraphicSize(Std.int(FlxMath.lerp(150, iconP1.width, Math.exp(-elapsed * 7))));
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(150, iconP2.width, Math.exp(-elapsed * 7))));

		iconP1.updateHitbox();
		iconP2.updateHitbox();

		var iconOffset:Int = 26;
		var center = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01));
		...
		if (generatedMusic)
		{
			...
			{
				var strum:StrumNote = (daNote.mustPress ? playerStrums : cpuStrums).members[daNote.noteData % 4];
				daNote.track(strum, SONG.speed);
				var _maxTime:Float = daNote.strumTime + daNote.sustainLength;
				...
				if (daNote.wasGoodHit && _maxTime < Conductor.songPosition)
				{
	...
	private function keyShit():Void
	{
		...
		if (boyfriend.holdTimer > Conductor.stepCrochet * 6 * 0.001 && !holdArray.contains(true))
		{
	...
	{
		var strum = playerStrums.members[note.noteData % 4];
		var _maxTime:Float = note.strumTime + note.sustainLength;
